...
const ClusterGenerator = require('./ClusterGenerator');
/**
...
function Timeline(container, items, groups, options) {
  ...
  this.body.emitter.on('updateItems', () => {
    if (this.options.cluster) {
      this._resetClusters();
      this.redraw();
    }
  ...
  this.body.emitter.on('removeItems', () => {
    if (this.options.cluster) {
      this._resetClusters();
      this.redraw();
    }
...
Timeline.prototype.setItems = function (items) {
  ...
  if (this.options.cluster) {
    this.clusterGenerator.clear();
    this.clusterGenerator.updateData({ applyOnChangedLevel: true });
  }
};
...
Timeline.prototype.setGroups = function (groups) {
  ...
  if (this.options.cluster) {
    this.clusterGenerator.clearCache();
    this.clusterGenerator.updateData({ applyOnChangedLevel: true });
  }
};
 ...
 * @param {Object} [options]  Available options:
 *                                `animation: boolean | {duration: number, easingFunction: string}`
 *                                    If true (default), the range is animat  ed
 *                                    smoothly to the new window. An object can be
 *                                    provided to specify duration and easing function.
 *                                    Default duration is 500 ms, and default easing
 *                                    function is 'easeInOutQuad'.
...
Timeline.prototype.clusterItems = function () {
  if (!this.options.cluster) {
    return;
  }
  ...
  const clusters = this.clusterGenerator.getClusters(scale, this.options.cluster);
...
Timeline.prototype._redraw = function () {
  if (this.options.cluster) {
    this.clusterItems();
  }
...
Timeline.prototype._onFit = function(args) {
...
Timeline.prototype._resetClusters = function() {
  if (this.options.cluster) {
    this.clusterGenerator.clear();
    this.clusterGenerator.updateData({ applyOnChangedLevel: false });
