 ...
 * @param {Object} timeline         parent timeline component instance
 * 
 */
...
ClusterGenerator.prototype.getClusters = function (oldClusters, scale, options) {
    const existingClustersLookup = (oldClusters || []).map(cluster => ({
        cluster,
        itemsIds: new Set(cluster.data.uiItems.map(item => item.id))
    }));

    let { maxItems, titleTemplate, clusterCriteria } = typeof options === "boolean" ? {} : options;
            ...
            if (this.groups.hasOwnProperty(groupName)) {
                ...
                while (i < iMax) {
                    ...
                    if (neighbors > maxItems) {
                        ...
                        let existingCluster;
                        if (existingClustersLookup.length) {
                            for (let clusterHolder of existingClustersLookup) {
                                if (clusterHolder.itemsIds.size === clusterItems.length 
                                    && clusterItems.every(clusterItem => clusterHolder.itemsIds.has(clusterItem.id))) {
                                    existingCluster = clusterHolder.cluster;
                                }
                            }
                        }

                        if (existingCluster) {
                            existingCluster.setUiItems(clusterItems);
                            clusters.push(existingCluster);
                        } else {
                            let cluster = this.createClusterItem({
                                'content': content,
                                'title': title,
                                'group': group,
