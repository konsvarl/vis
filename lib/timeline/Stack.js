 ...
 */
exports.stack = function(orderedItems, margin, force) {
  if (force) {
    // reset top position of all items
    for (var i = 0; i < orderedItems.length; i++) {
      orderedItems[i].top = null;
    }
  }

  // calculate new, non-overlapping positions
  for (let i = 0; i < orderedItems.length; i++) {  // eslint-disable-line no-redeclare
    const item = orderedItems[i];
    if (item.stack && item.top === null) {
      // initialize top position
      item.top = margin.axis;

      const interval = item.data.end && item.data.start 
                      ? Math.abs(item.data.end.valueOf() - item.data.start.valueOf())
                      : null;
      ...
      do {
        ...
        for (let j = i +1; j < orderedItems.length; j++) {
          const other = orderedItems[j];
          if (interval) {
            // if (item.data.end && other.data.start.valueOf() - item.data.end.valueOf() > interval / 2) {
            //   break;
            // }
  
            // if (other.data.end && item.data.start.valueOf() - other.data.end.valueOf() > interval / 2) {
            //   break;
            // }
          }

          if (other.top !== null && other.stack && exports.collision(item, other, margin.item, other.options.rtl)) {
            collidingItem = other;
            break;
          }
        }
        ...
        if (!collidingItem) {
          for (let j = i - 1; j >= 0; j--) {
            ...
            if (interval) {
              // if (item.data.end && other.data.start.valueOf() - item.data.end.valueOf() > interval) {
              //   break;
              // }
    
              // if (other.data.end && item.data.start.valueOf() - other.data.end.valueOf() > interval) {
              //   break;
              // }
            }
