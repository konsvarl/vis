...
ClusterItem.prototype._setupRange = function () {
  ...
  this.data.fitStart = this.data.min;
  this.data.fitEnd = this.data.min + (this.data.max - this.data.min)/2;
}
...
ClusterItem.prototype._fit = function() {
  if (this.emitter) {
    const fitArgs = {
      start: new Date(this.data.fitStart),
      end: new Date(this.data.fitEnd),
      animation: true
    };
