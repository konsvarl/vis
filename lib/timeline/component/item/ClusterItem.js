...
ClusterItem.prototype._isCoordinateVisible = function() {
  return this.minWidth >= this.width;
}

ClusterItem.prototype._getFitRange = function() {
  if (this.data.min >= this.range.start && this.data.max <= this.range.end) {
    return {
      fitStart: this.data.min,
      fitEnd: this.data.max
    };
  }

  const rangeCenter = this.range.start + (this.range.end - this.range.start) / 2;

  if (this.data.max > this.range.end) {
    if (this.data.min < rangeCenter && this.data.min >= this.range.start) {
      return {
        fitStart: this.data.min - 0.05*(rangeCenter - this.data.min) / 2,
        fitEnd: rangeCenter
      };
    } else {
      return {
        fitStart: this.data.min - 0.05*(this.range.end - this.data.min) / 2,
        fitEnd: this.range.end
      };
    }
  } else {
    if (this.data.max <= rangeCenter) {
      return {
        fitStart: this.range.start,
        fitEnd: this.data.max + 0.05*(this.data.max - this.range.start) / 2
      };
    } else {
      return {
        fitStart: rangeCenter,
        fitEnd: this.data.max + 0.05*(this.data.max - rangeCenter) / 2
      };
    }
  }
}

ClusterItem.prototype._fit = function() {
  if (this.emitter) {
