...
Group.prototype._redrawItems = function(forceRestack, lastIsVisible, margin, range) {
   ...
   */
    ...
    if (typeof this.itemSet.options.order === 'function') {
      ...
      //   if (!item.displayed && !item.cluster) {
      ...
      if (this.doInnerStack && this.itemSet.options.stackSubgroups) {
        ...
        stack.stack(customOrderedItems, margin, true /* restack=true */, true);
      }
    } else {
 ...
 * @param {boolean} batch
 */
Group.prototype.add = function(item, batch) {
  ...
  if (!batch && this.visibleItems.indexOf(item) == -1) {
...
Group.prototype._updateClustersInRange = function(orderedClusters, oldVisibleClusters, range) {
  ...
  for (i = 0; i < visibleClusters.length; i++) {
    var item = visibleClusters[i];
    if (!item.displayed) {
      var returnQueue = true;
      redrawQueue[i] = item.redraw(returnQueue);
      redrawQueueLength = redrawQueue[i].length;
